...
export default function AdminPage() {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-healthcare-600 via-healthcare-700 to-purple-900 flex items-center justify-center p-4">
        <div className="w-full max-w-md">
          <div className="bg-white rounded-3xl shadow-2xl p-10 space-y-8">
            {/* Login Form */}
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="relative">
                <label className="block text-sm font-bold text-healthcare-800 mb-3">Admin Password</label>
                <input
                  type="password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  placeholder="Enter your password"
                  className="w-full px-5 py-4 border-2 border-healthcare-200 rounded-xl focus:outline-none focus:border-healthcare-500 focus:ring-2 focus:ring-healthcare-200 text-base transition-all"
                  autoFocus
                />
              </div>
              <button
            ...
            {/* Password Hint */}
            <div className="bg-gradient-to-r from-blue-50 to-blue-100 border-2 border-blue-300 rounded-xl p-5 space-y-2">
              <p className="text-sm font-bold text-blue-900 flex items-center gap-2">
                <span className="text-lg">💡</span> Demo Credentials
              </p>
              <div className="bg-white rounded-lg px-4 py-2 font-mono font-bold text-blue-600 text-center">
                admin123
              </div>
              <p className="text-xs text-blue-800 text-center">For demo purposes only</p>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
            ...
            <button
              ...
              className={`px-6 py-3 rounded-full font-bold whitespace-nowrap transition-all transform ${
                activeTab === tab
                  ...
                  : 'bg-white text-gray-700 hover:bg-healthcare-50 border-2 border-gray-200'
              }`}
            >
        ...
        {activeTab === 'doctors' && (
          <div className="animate-fade-in">
            <div className="bg-white rounded-3xl border-2 border-healthcare-200 shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-healthcare-100 to-blue-50 p-8 border-b border-healthcare-200">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-3xl font-black text-healthcare-800">Manage Doctors</h2>
                    <p className="text-gray-600 mt-2">{doctors.length} doctors in the system</p>
                  </div>
              ...
              <div className="p-8">
                {doctors.length === 0 ? (
                  <div className="text-center py-12 space-y-4">
                    ...
                    <p className="text-gray-500 text-lg font-semibold">No doctors yet</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {doctors.map(doctor => (
                      <div key={doctor._id} className="group flex items-center justify-between p-6 border-2 border-healthcare-100 rounded-2xl hover:border-healthcare-500 hover:bg-healthcare-50 transition-all">
                        <div className="flex-1">
                          <p className="font-bold text-lg text-healthcare-800">{doctor.name}</p>
                          <p className="text-sm text-gray-600 mt-1 font-semibold">{doctor.specialty}</p>
                          <p className="text-xs text-gray-500 mt-2">⭐ {doctor.rating} • {doctor.experience_years} years exp • {doctor.qualifications}</p>
                        </div>
                        <button
          ...
          <div className="animate-fade-in">
            <div className="bg-white rounded-3xl border-2 border-healthcare-200 shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-healthcare-100 to-blue-50 p-8 border-b border-healthcare-200">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-3xl font-black text-healthcare-800">All Bookings</h2>
                    <p className="text-gray-600 mt-2">{bookings.length} appointments scheduled</p>
                  </div>
                  <div className="text-5xl">📅</div>
                </div>
              </div>
              <div className="p-8 overflow-x-auto">
                {bookings.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gradient-to-r from-healthcare-100 to-blue-50 border-b-2 border-healthcare-200">
                        <th className="text-left py-4 px-4 font-bold text-healthcare-800">Doctor</th>
                        <th className="text-left py-4 px-4 font-bold text-healthcare-800">Patient Email</th>
                        <th className="text-left py-4 px-4 font-bold text-healthcare-800">Phone</th>
                        <th className="text-left py-4 px-4 font-bold text-healthcare-800">Appointment</th>
                        <th className="text-center py-4 px-4 font-bold text-healthcare-800">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      {bookings.map(booking => (
                        <tr key={booking._id} className="border-b border-gray-200 hover:bg-healthcare-50 transition-all">
                          <td className="py-4 px-4 font-semibold text-healthcare-700">{booking.doctor_name || 'Unknown'}</td>
                          <td className="py-4 px-4 text-gray-600">{booking.patient_email}</td>
                          <td className="py-4 px-4 text-gray-600">{booking.patient_phone}</td>
          ...
          <div className="animate-fade-in">
            <div className="bg-white rounded-3xl border-2 border-healthcare-200 shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-healthcare-100 to-blue-50 p-8 border-b border-healthcare-200">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-3xl font-black text-healthcare-800">System Statistics</h2>
              ...
              <div className="p-8">
                {stats.length === 0 ? (
                  <div className="text-center py-12 space-y-4">
                    <div className="text-5xl">📭</div>
                    <p className="text-gray-500 text-lg font-semibold">No statistics available</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {stats.map(stat => (
                      <div key={stat.doctor_id} className="bg-gradient-to-br from-white to-healthcare-50 rounded-2xl p-6 border-2 border-healthcare-100 hover:border-healthcare-500 transition-all group">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <p className="font-black text-healthcare-700 text-lg">{stat.doctor_name}</p>
                            <p className="text-xs text-gray-500 mt-1">Doctor Analytics</p>
                          </div>
                          <div className="text-3xl group-hover:scale-125 transition-transform">👨‍⚕️</div>
                        </div>
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600 font-semibold">Total Slots:</span>
